...
Mg_rec2 <- recipe(Mg ~ OP_Age + Thick + Season + D_Canal + D_OPT + Depth,
                      data = train_data_Mg) %>%
...
Mg_rec_prepped2 <- prep(Mg_rec2, training = train_data_Mg)

# 4. Process datasets ---------------------------------------------------------
train_data_processed_Mg2 <- bake(Mg_rec_prepped2, new_data = train_data_Mg)
test_data_processed_Mg2 <- bake(Mg_rec_prepped2, new_data = test_data_Mg)

# 5. Verify processed data structure ------------------------------------------
glimpse(train_data_processed_Mg2)

# 6. Lightweight Model Spec ---------------------------------------------------
mlp_spec_tune_Mg2 <- mlp(
  epochs = tune(),
  hidden_units = tune(),
  penalty = tune(),
  learn_rate = tune()
...
# 7. Minimal Workflow --------------------------------------------------------
mlp_wflow_tune_Mg2 <- workflow() %>%
  add_recipe(Mg_rec2) %>%
  add_model(mlp_spec_tune_Mg2)
...
set.seed(123)
folds_Mg2 <- vfold_cv(train_data_Mg, v = 5)

set.seed(123)
param_grid_Mg2 <- grid_latin_hypercube(
...
show_best(grid_results_Mg2, n = 10, metric = "rmse")

save.image(file='E://Fajrin/Publikasi/Pak Heru B Pulunggono/0 Road to Prof/18 Predicting Macronutrient in peat using ML/Data_Private/modelling_mlp2_19022025_Mg2.RData')

autoplot(grid_results_Mg2)