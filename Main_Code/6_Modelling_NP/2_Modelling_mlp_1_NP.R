...
NP_rec1 <- recipe(NP ~ OP_Age + Thick + Season + D_Canal + D_OPT + Depth,
                      data = train_data_NP) %>%
...
NP_rec_prepped1 <- prep(NP_rec1, training = train_data_NP)

# 4. Process datasets ---------------------------------------------------------
train_data_processed_NP1 <- bake(NP_rec_prepped1, new_data = train_data_NP)
test_data_processed_NP1 <- bake(NP_rec_prepped1, new_data = test_data_NP)

# 5. Verify processed data structure ------------------------------------------
glimpse(train_data_processed_NP1)

# 6. Lightweight Model Spec ---------------------------------------------------
mlp_spec_tune_NP1 <- mlp(
  epochs = tune(),
  hidden_units = tune(),
  penalty = tune(),
  learn_rate = tune()
...
# 7. Minimal Workflow --------------------------------------------------------
mlp_wflow_tune_NP1 <- workflow() %>%
  add_recipe(NP_rec1) %>%
  add_model(mlp_spec_tune_NP1)
...
set.seed(123)
folds_NP1 <- vfold_cv(train_data_NP, v = 5)

set.seed(123)
param_grid_NP1 <- grid_latin_hypercube(
...
show_best(grid_results_NP1, n = 10, metric = "rmse")


save.image(file='E://Fajrin/Publikasi/Pak Heru B Pulunggono/0 Road to Prof/18 Predicting Macronutrient in peat using ML/Data_Private/modelling_mlp2_19022025_NP1.RData')

autoplot(grid_results_NP1)