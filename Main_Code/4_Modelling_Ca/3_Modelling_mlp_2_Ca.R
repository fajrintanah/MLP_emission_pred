...
# 1. Splitting data--------------------------------------------------------------
set.seed(123)
data_split_Ca <- initial_split(EL_Data_Ca2, prop = 0.7)
train_data_Ca <- training(data_split_Ca)
test_data_Ca <- testing(data_split_Ca)

# 2. Recipe Setup (Corrected) -------------------------------------------------
# First define the recipe without immediate prep()
Ca_rec2 <- recipe(Ca ~ OP_Age + Thick + Season + D_Canal + D_OPT + Depth,
                      data = train_data_Ca) %>%
...
Ca_rec_prepped2 <- prep(Ca_rec2, training = train_data_Ca)

# 4. Process datasets ---------------------------------------------------------
train_data_processed_Ca2 <- bake(Ca_rec_prepped2, new_data = train_data_Ca)
test_data_processed_Ca2 <- bake(Ca_rec_prepped2, new_data = test_data_Ca)

# 5. Verify processed data structure ------------------------------------------
glimpse(train_data_processed_Ca2)

# 6. Lightweight Model Spec ---------------------------------------------------
mlp_spec_tune_Ca2 <- mlp(
  epochs = tune(),
  hidden_units = tune(),
  penalty = tune(),
  learn_rate = tune()
...
# 7. Minimal Workflow --------------------------------------------------------
mlp_wflow_tune_Ca2 <- workflow() %>%
  add_recipe(Ca_rec2) %>%
  add_model(mlp_spec_tune_Ca2)
...
set.seed(123)
folds_Ca2 <- vfold_cv(train_data_Ca, v = 5)

set.seed(123)
param_grid_Ca2 <- grid_latin_hypercube(
...
show_best(grid_results_Ca2, n = 10, metric = "rmse")

save.image(file='E://Fajrin/Publikasi/Pak Heru B Pulunggono/0 Road to Prof/18 Predicting Macronutrient in peat using ML/Data_Private/modelling_mlp2_19022025_Ca2.RData')

autoplot(grid_results_Ca2)
