...
Ca_rec1 <- recipe(Ca ~ OP_Age + Thick + Season + D_Canal + D_OPT + Depth,
                      data = train_data_Ca) %>%
...
Ca_rec_prepped1 <- prep(Ca_rec1, training = train_data_Ca)

# 4. Process datasets ---------------------------------------------------------
train_data_processed_Ca1 <- bake(Ca_rec_prepped1, new_data = train_data_Ca)
test_data_processed_Ca1 <- bake(Ca_rec_prepped1, new_data = test_data_Ca)

# 5. Verify processed data structure ------------------------------------------
glimpse(train_data_processed_Ca1)

# 6. Lightweight Model Spec ---------------------------------------------------
mlp_spec_tune_Ca1 <- mlp(
  epochs = tune(),
  hidden_units = tune(),
  penalty = tune(),
  learn_rate = tune()
...
# 7. Minimal Workflow --------------------------------------------------------
mlp_wflow_tune_Ca1 <- workflow() %>%
  add_recipe(Ca_rec1) %>%
  add_model(mlp_spec_tune_Ca1)
...
set.seed(123)
folds_Ca1 <- vfold_cv(train_data_Ca, v = 5)

set.seed(123)
param_grid_Ca1 <- grid_latin_hypercube(
...
show_best(grid_results_Ca1, n = 10, metric = "rmse")


save.image(file='E://Fajrin/Publikasi/Pak Heru B Pulunggono/0 Road to Prof/18 Predicting Macronutrient in peat using ML/Data_Private/modelling_mlp2_19022025_Ca1.RData')

autoplot(grid_results_Ca1)
