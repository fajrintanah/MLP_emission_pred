---
title: "Macronutrient Prediction in Peat using ML"
author: "Your Name"
date: "`r Sys.Date()`"
...
EL_Data_N <- Data_N %>% 
  ...
  mutate(N = ifelse(qc_N == 0, NA_real_, N)) %>% 
  drop_na() %>% 
  select(-qc_N) # Remove quality control column
...
```{r}
set.seed(123)
data_split_N <- initial_split(EL_Data_N, prop = 0.7)
...
```{r}
EL_Data_N2 <- dummy_cols(EL_Data_N, 
                         select_columns = c("OP_Age", "Thick", "Season"), 
                         remove_selected_columns = TRUE)

str(EL_Data_N2)
```

### Train-Test Split with Status Column
...
N_train <- trainSet_N %>% mutate(Status = "TrainingN")
N_test <- testSet_N %>% mutate(Status = "ValidationN")
...
```{r}
# Assume x_N_train and y_N_train are preloaded datasets
```
...
# My beloved wife, I dont like the repetitive codes like above. it is soo boring 
# Lets check if this repetitive processes can be automatically done by a wrapper function 
...
## dont plot all VI scores. just taking the strongest models based on calibration and validation methods, plot their VI scores
...
## it is easy when we have a simplified, pre-determined function. such a neat code, yeay, we nailed it !!!
...
## yess, we can get rid of many repetitive codes wkwkwkwkw 
